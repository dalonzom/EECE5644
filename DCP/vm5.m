% Voting Method 5 - VM4 with active learning for validation data 
function [results] = vm5(test, guess, numAttributes, training_set, intervalCap, votingCap)
results = struct;
classification = [];
...
for j = 1:size(test,1)
    ...
    for i = 1:numAttributes
        if (guess{j}.(names{i}) ~= 0)
            ...
            training_set{i}.intervalCounts(guess{j}.(names{i+numAttributes}),test(j, numAttributes + 1)) = training_set{i}.intervalCounts(guess{j}.(names{i+numAttributes}),test(j, numAttributes + 1)) + 1; 
        end
